	...
	.directive('ceRightPane',  function()
	{
		return {
			...
			template: "<div> \
						...
						<div ng-if=\"isConnected()\" class=\"ce-item ce-folder dropzone\" data-path=\"{{ filePath = path }}\"> \
...
//								<a ng-hide=\"file.is_dir\" ng-href=\"{{download()}}\" download=\"{{file.name}}\" target=\"blank\">download</a> \
	...
	.directive('ceBrowser',  function()
	{
		return {
			...
			replace: true,
			template: "<div> \
						<div class=\"ceTitle\">Browse your cloud drives <button class=\"close-btn\" type=\"button\" ng-click=\"abort()\"></button> <button type=\"button\" ng-click=\"logout()\" ng-if=\"isConnected()\">({{ getDisplayName() }}) Logout</button></div> \
